//https://github.com/TooTallNate/node-telnet/blob/master/lib/telnet.js#L15
var net = require('net')
...
function parseOption (bufs, i, event) {
  ...
  hook=OPTION_IMPLS[option]
  if (hook==null){hook=OPTION_IMPLS.NO_ARGS};
  return hook(bufs, i + 1, event)
}
...
function Socket (input, output) {
  ...
  this.input.on('data', function (b) {
    debug('incoming "data" event %j', b.toString(''), b)
    bufs.push(b);
    var i
